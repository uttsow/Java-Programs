...
//		}
		...
		try (PrintWriter output = new PrintWriter("lastNames.txt")){
			Stream<String> lstName = Arrays.stream(lastNames);
			lstName.forEach(output::println);
		}
		
		try (PrintWriter output = new PrintWriter("male1stNames.txt")){
			Stream<String> male = Arrays.stream(maleFirstNames);
			male.forEach(output::println);
		}
		try (PrintWriter output = new PrintWriter("female1stNames.txt")){
			Stream<String> female = Arrays.stream(femaleFirstNames);
			female.forEach(output::println);
		}
		
		try (PrintWriter output = new PrintWriter("roadName.txt")){
			Stream<String> road = Arrays.stream(roadNames);
			road.forEach(output::println);
		}
		try (PrintWriter output = new PrintWriter("townName.txt")){
			Stream<String> town = Arrays.stream(townNames);
			town.forEach(output::println);
		}
		try (PrintWriter output = new PrintWriter("country.txt")){
			for(String city : capitals.keySet()) {
			//System.out.println(capitals.get(city));
			output.println(capitals.get(city));
			}
		}
